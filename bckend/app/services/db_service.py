...
def get_all_cv_data():
    conn = psycopg2.connect(**DB_CONFIG)
    cursor = conn.cursor()
    cursor.execute("SELECT name, email, phone, location, education, skills, experience_years, current_company, expected_salary, notice_period, portfolio_link FROM cv_data")
